...
public class Picture extends SimplePicture 
{
		...
		Pixel rightPixel = null;
			...
			for (int r=0; r<pixels.length;r++) {
				for(int c=0; c<pixels[r].length/2;c++) {
					Color top=pixels[r][c].getColor();
					pixels[r][c].setColor(pixels[r][pixels[r].length-c-1].getColor());
					pixels[r][pixels[r].length-c-1].setColor(top);
				}
			}
	}
	 ...
	 */
	public void edgeDetection(double edgeDist)
	{
		...
		for (int r=0; r<pixels.length; r++)
		{
			for (int c=0; c<pixels[r].length;c++) 
			{
				...
				if (c<pixels[r].length-1) 
				{
					...
					double dist= leftPixel.colorDistance(rightPixel.getColor());
					if(dist>edgeDist){
						leftPixel.setBlue(0);
